...
static counter_t sim_num_RAW_hazard_q1_stall_1;
static counter_t sim_num_RAW_hazard_q1_stall_2;
static counter_t sim_num_RAW_hazard_q2;
static counter_t sim_num_RAW_hazard_q2_stall_1;
static counter_t sim_num_RAW_hazard_q2_stall_2;
...
//static counter_t wb_stage_ready2;
int wb_stage_ready =0;
int wb_stage_ready2 = 0;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q1_stall_1",
		   "total number of RAW hazards with 1 cycle stall (q1)",
		   &sim_num_RAW_hazard_q1_stall_1, sim_num_RAW_hazard_q1_stall_1, NULL);
  
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q1_stall_2",
		   "total number of RAW hazards with 2 cycle stall(q1)",
		   &sim_num_RAW_hazard_q1_stall_2, sim_num_RAW_hazard_q1_stall_2, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2",
		   "total number of RAW hazards (q2)",
		   &sim_num_RAW_hazard_q2, sim_num_RAW_hazard_q2, NULL);
  
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2_stall_1",
		   "total number of RAW hazards with 1 cycle stall (q2)",
		   &sim_num_RAW_hazard_q2_stall_1, sim_num_RAW_hazard_q2_stall_1, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2_stall_2",
		   "total number of RAW hazards with 2 cycle stall (q2)",
		   &sim_num_RAW_hazard_q2_stall_2, sim_num_RAW_hazard_q2_stall_2, NULL);

  stat_reg_counter(sdb, "sim_num_WAW_hazard_q3",
		   "total number of WAW hazards (q3)",
		   &sim_num_WAW_hazard_q3, sim_num_WAW_hazard_q3, NULL);

  stat_reg_counter(sdb, "sim_num_WAW_hazard_q3_stall_1",
		   "total number of WAW hazards with 1 cycle stall(q3)",
		   &sim_num_WAW_hazard_q3_stall_1, sim_num_WAW_hazard_q3_stall_1, NULL);

  stat_reg_counter(sdb, "sim_num_WAW_hazard_q3_stall_2",
		   "total number of WAW hazards with 2 cycle stall(q3)",
		   &sim_num_WAW_hazard_q3_stall_2, sim_num_WAW_hazard_q3_stall_2, NULL);
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   ...
		   "(sim_num_insn+sim_num_RAW_hazard_q2_stall_1+2*sim_num_RAW_hazard_q2_stall_2)/sim_num_insn", NULL);
...
/* ECE552 Assignment 1 - END CODE*/
  ...
  while (TRUE)
	...
	{
/* ECE552 Assignment 1 - BEGIN CODE*/
//keeps track of registers that we are using for input and output
...
//            printf("UNICYCLE r_in[0]= %d, r_in[1]= %d, r_in[2]= %d, r_out[0]= %d, r_out[1]= %d, sim_num_insn= %d, reg_ready[i]= %d, i= %d\n", r_in[0], r_in[1], r_in[2], r_out[0], r_out[1], sim_num_insn, reg_ready_q1[r_in[i]], i);
  ...
  //          printf("BICYCLE r_in[0]= %d, r_in[1]= %d, r_in[2]= %d, r_out[0]= %d, r_out[1]= %d, sim_num_insn= %d, reg_ready[i]= %d, i= %d\n", r_in[0], r_in[1], r_in[2], r_out[0], r_out[1], sim_num_insn, reg_ready_q1[r_in[i]], i);
	 ...
	 if (r_out[i] != DNA && reg_ready_q3 [r_out [i]] == sim_num_insn+1 && !(MD_OP_FLAGS(op)&F_MEM) ) {
	    ...
	    //if(wb_stage_ready2==0)
	    	sim_num_structural_hazard_q3++;
	    if(wb_stage_ready2==1) //2 mem operations followed by an alu op causes hazard with first op and then on second after stall
	    	sim_num_structural_hazard_q3++;
	    wb_stage_ready = 0;
	    wb_stage_ready2 = 0;
	    break;
	 ...
	 reg_ready_q2[r_out[1]] = sim_num_insn + 2;
      ...
      if (r_out[0] != DNA && ((MD_OP_FLAGS(op)&F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)))
...
//            printf(" sim_num_insn= %d, wb_stage_ready= %d \n",  sim_num_insn, wb_stage_ready);
 //checks to see for structural hazard. also checking to see if there is already a potential structural hazard 
      if (MD_OP_FLAGS(op)&F_MEM && wb_stage_ready < sim_num_insn)
	 
         wb_stage_ready = sim_num_insn + 3;
	        
      if (MD_OP_FLAGS(op)&F_MEM && wb_stage_ready==sim_num_insn+2)
         wb_stage_ready2=1;
      /* ECE552 Assignment 1 - END CODE*/
